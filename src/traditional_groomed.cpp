...
int checkGroom(int id)
{
    cout << "Checking for similar request for id = " << id << endl;
    int req = -1, minSlots = INT_MAX;
    for (int i = 0; i < id; i++)
    {
        if (requestsInfo[i][NO_OF_DEST + 1] == 1) //if req is active
        {
            if (requestsInfo[i][0] == requestsInfo[id][0] && requestsInfo[i][1] == requestsInfo[id][1] && requestsInfo[i][2] == requestsInfo[id][2] && requestsInfo[i][3] == requestsInfo[id][3])
            {
                if (requestsInfo[i][NO_OF_DEST + 4] < minSlots)
                {
                    minSlots = requestsInfo[i][NO_OF_DEST + 4];
                    req = i;
                }
            }
        }
...
int generateRequest(int &bt, int &src, int requestId)
{
    ...
    // requestsInfo[requestId][0] = 10;
    // requestsInfo[requestId][1] = 18;
    // requestsInfo[requestId][2] = 6;
    // requestsInfo[requestId][3] = 8;
...
int findBestPolicy(struct slotType (&slotMatrixClone)[VERTICES][VERTICES], set<pair<int, int>> lightTree, set<pair<int, int>> lightTreeBackup, int requiredSlots)
{
...
int allocateSlotsGroom(set<pair<int, int>> lightTree, set<pair<int, int>> lightTreeBackup, int requiredSlots, int &beginIndexOriginal, int &beginIndexBackup, int req_id, int groom_id)
{
    //cloning the shared memroy slots
    slotType slotMatrixClone[VERTICES][VERTICES];
    cloneSlotMatrix(slotMatrixClone);

    int groom_beginIndexOriginal = requestsInfo[groom_id][NO_OF_DEST + 2];
    ...
    if(allocationStatusOriginal.first==0 && allocationStatusBackup.first ==0){
        ...
        //deallocate slots from main slot matrix to start grooming
        deallocationMainSlotMatrix(lightTree, groom_beginIndexOriginal, groomReqSlots + 2);
        deallocationMainSlotMatrix(lightTreeBackup, groom_beginIndexBackup, groomReqSlots + 2);
...
int main()
{
    ...
    for (int i = 0; i < noOfRequests; i++)
    {
        ...
        else
        {
            ...
            groom_id = checkGroom(id);
            if (groom_id != -1)
            {
                noOfSlotsReq += requestsInfo[groom_id][NO_OF_DEST + 4];
                groom_status = allocateSlotsGroom(lightTree, lightTreeBackup, noOfSlotsReq + 2, beginIndexOriginal, beginIndexBackup, id, groom_id);
                isSlotsAllocated = groom_status;
                if (groom_status == 1){
                    requestsInfo[groom_id][NO_OF_DEST + 1]=3;
                    cout << "Request id: " << id << " GROOMED SUCCESSFULLY with req id: " << groom_id << endl;
                }
                else
                    noOfSlotsReq=noOfSlotsReq-requestsInfo[groom_id][NO_OF_DEST + 4];
            }
            if (groom_status != 1) //if grooming fails, perform normal allocation
            {
                //slot allocation done here
        ...
        if (isSlotsAllocated == 1) // SLOTS successfully allocated for the request
        {
            ...
            if(requestsInfo[id][NO_OF_DEST + 1] != 3)
                requestsInfo[id][NO_OF_DEST + 1] = 4;   // 4 means completed
            
            if( groom_status ==1 )
                simulation_info[2] += noOfSlotsReqOriginal;
            else
                simulation_info[2] += noOfSlotsReq + 2; // simulation_info[2] : no of SLOTS allocated

            //SLOTS DEALLOCATION

            if (requestsInfo[id][NO_OF_DEST + 1] != 3) //if the process is not groomed, then perform deallocation
            {
